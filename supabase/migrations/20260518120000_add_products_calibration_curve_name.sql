ALTER TABLE public.plant_products_config
  ADD COLUMN IF NOT EXISTS calibration_curve_name TEXT;
    ...
    WHEN 'aggregates' THEN
      ...
      IF jsonb_array_length(v_rows) > 0 THEN
        INSERT INTO public.plant_aggregates_config (
          id, plant_id, aggregate_name, material_type, location_area, measurement_method,
          unit, box_width_ft, box_height_ft, sort_order, is_active
        )
        SELECT
          ...
          COALESCE(row_data.measurement_method, 'BOX'),
    ...
    WHEN 'cajones' THEN
      ...
      IF jsonb_array_length(v_rows) > 0 THEN
        INSERT INTO public.plant_cajones_config (
          id, plant_id, cajon_name, material, procedencia, box_width_ft, box_height_ft,
    ...
    WHEN 'additives' THEN
      ...
      IF jsonb_array_length(v_rows) > 0 THEN
        ...
        SELECT
          ...
          COALESCE(row_data.measurement_method, 'MANUAL_QUANTITY'),
    ...
    WHEN 'diesel' THEN
      ...
      IF jsonb_array_length(v_rows) > 0 THEN
        INSERT INTO public.plant_diesel_config (
          id, plant_id, measurement_method, calibration_curve_name, reading_uom,
          tank_capacity_gallons, initial_inventory_gallons, calibration_table, sort_order, is_active
        )
        SELECT
          COALESCE(row_data.id, gen_random_uuid()::text),
          p_plant_id,
          COALESCE(row_data.measurement_method, 'TANK_LEVEL'),
          row_data.calibration_curve_name,
          row_data.reading_uom,
    ...
    WHEN 'products' THEN
      ...
      IF jsonb_array_length(v_rows) > 0 THEN
        INSERT INTO public.plant_products_config (
          ...
          calibration_curve_name, reading_uom, calibration_table, tank_capacity, unit_volume,
