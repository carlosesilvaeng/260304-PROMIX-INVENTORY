import {
  ADDITIVES_CATALOG_IMPORT_COLUMNS,
  ADDITIVES_CATALOG_IMPORT_META_SHEET,
  ADDITIVES_CATALOG_IMPORT_MODULE,
  ADDITIVES_CATALOG_IMPORT_SHEET_NAME,
  ADDITIVES_CATALOG_IMPORT_TEMPLATE_VERSION,
  type AdditivesCatalogImportMeta,
} from './additivesCatalogImportWorkbook';

export interface ParsedAdditivesCatalogImportRow {
  row_number: number;
  nombre: string;
  marca: string;
  uom: string;
...
export interface ParsedAdditivesCatalogImportFile {
  meta: AdditivesCatalogImportMeta;
  rows: ParsedAdditivesCatalogImportRow[];
}
...
export async function parseAdditivesCatalogImportFile(file: File): Promise<ParsedAdditivesCatalogImportFile> {
  const XLSX = await import('xlsx');
  const buffer = await file.arrayBuffer();
  const workbook = XLSX.read(buffer, { type: 'array' });

  const dataSheet = workbook.Sheets[ADDITIVES_CATALOG_IMPORT_SHEET_NAME];
  if (!dataSheet) {
    throw new Error(`El archivo no contiene la hoja "${ADDITIVES_CATALOG_IMPORT_SHEET_NAME}".`);
  }
  ...
  const expectedHeaders = ADDITIVES_CATALOG_IMPORT_COLUMNS.map((column) => column.label);
  ...
  const rows: ParsedAdditivesCatalogImportRow[] = [];
  headerRows.slice(1).forEach((rowValues, index) => {
    const normalized = ADDITIVES_CATALOG_IMPORT_COLUMNS.reduce((acc, column, columnIndex) => {
  ...
  const metaSheet = workbook.Sheets[ADDITIVES_CATALOG_IMPORT_META_SHEET];
  if (!metaSheet) {
  ...
  if (metaMap.module !== ADDITIVES_CATALOG_IMPORT_MODULE) {
    throw new Error('La plantilla no corresponde al catálogo de Aditivos.');
  }

  if (metaMap.template_version !== ADDITIVES_CATALOG_IMPORT_TEMPLATE_VERSION) {
