import {
  PROCEDENCIAS_IMPORT_COLUMNS,
  PROCEDENCIAS_IMPORT_META_SHEET,
  PROCEDENCIAS_IMPORT_MODULE,
  PROCEDENCIAS_IMPORT_SHEET_NAME,
  PROCEDENCIAS_IMPORT_TEMPLATE_VERSION,
  type ProcedenciasImportMeta,
} from './procedenciasImportWorkbook';

export interface ParsedProcedenciasImportRow {
  row_number: number;
  nombre: string;
}

export interface ParsedProcedenciasImportFile {
  meta: ProcedenciasImportMeta;
  rows: ParsedProcedenciasImportRow[];
}
...
export async function parseProcedenciasImportFile(file: File): Promise<ParsedProcedenciasImportFile> {
  const XLSX = await import('xlsx');
  const buffer = await file.arrayBuffer();
  const workbook = XLSX.read(buffer, { type: 'array' });

  const dataSheet = workbook.Sheets[PROCEDENCIAS_IMPORT_SHEET_NAME];
  if (!dataSheet) {
    throw new Error(`El archivo no contiene la hoja "${PROCEDENCIAS_IMPORT_SHEET_NAME}".`);
  }
  ...
  const expectedHeaders = PROCEDENCIAS_IMPORT_COLUMNS.map((column) => column.label);
  ...
  const rows: ParsedProcedenciasImportRow[] = [];
  headerRows.slice(1).forEach((rowValues, index) => {
    const normalized = PROCEDENCIAS_IMPORT_COLUMNS.reduce((acc, column, columnIndex) => {
  ...
  const metaSheet = workbook.Sheets[PROCEDENCIAS_IMPORT_META_SHEET];
  if (!metaSheet) {
  ...
  if (metaMap.module !== PROCEDENCIAS_IMPORT_MODULE) {
    throw new Error('La plantilla no corresponde al catálogo de Procedencias.');
  }

  if (metaMap.template_version !== PROCEDENCIAS_IMPORT_TEMPLATE_VERSION) {
